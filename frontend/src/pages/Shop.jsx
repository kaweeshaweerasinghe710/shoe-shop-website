...
const Shop = () => {
  ...
  const loadCategories = async () => {
    try {
      setLoading(true);
      const res = await fetch((import.meta.env.VITE_API_URL || 'https://strong-courage-production.up.railway.app/api') + '/categories'); // your backend URL
...
const loadCategories = async () => {
  try {
    const res = await fetch((import.meta.env.VITE_API_URL || 'https://strong-courage-production.up.railway.app/api') + '/categories');
    if (!res.ok) throw new Error('Failed to fetch');
    const data = await res.json();
