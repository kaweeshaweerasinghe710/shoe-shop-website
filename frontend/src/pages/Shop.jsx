...
const Shop = () => {
  ...
  const loadCategories = async () => {
    try {
      setLoading(true);
      const res = await fetch('https://shoe-shop-website-m48s.vercel.app/api/categories'); // your backend URL
...
const loadCategories = async () => {
  try {
    const res = await fetch('https://shoe-shop-website-m48s.vercel.app/api/categories');
    if (!res.ok) throw new Error('Failed to fetch');
    const data = await res.json();
